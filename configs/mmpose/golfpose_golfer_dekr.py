...
visualizer = dict(type="PoseLocalVisualizer", vis_backends=[dict(type="LocalVisBackend"), dict(type="TensorboardVisBackend")], name="visualizer")
...
model = dict(
    ...
    head=dict(
        ...
        displacement_loss=dict(type="SoftWeightSmoothL1Loss", use_target_weight=True, supervise_empty=False, beta=0.1111111111111111, loss_weight=0.002),
        decoder=dict(
            type="SPR",
            input_size=(640, 640),
            heatmap_size=(160, 160),
...
train_pipeline = [
    ...
    dict(type="GenerateTarget", encoder=dict(type="SPR", input_size=(640, 640), heatmap_size=(160, 160), sigma=(4, 2), minimal_diagonal_length=5.656854249492381, generate_keypoint_heatmaps=True, decode_max_instances=30)),
    dict(type="BottomupGetHeatmapMask"),
    dict(type="PackPoseInputs"),
]
val_pipeline = [
    dict(type="LoadImage"),
    dict(type="BottomupResize", input_size=(640, 640), size_factor=32, resize_mode="expand"),
    dict(type="PackPoseInputs", meta_keys=("id", "img_id", "img_path", "crowd_index", "ori_shape", "img_shape", "input_size", "input_center", "input_scale", "flip", "flip_direction", "flip_indices", "raw_ann_info", "skeleton_links")),
]

dataset_type = "CocoDataset"
# dataset_type = "GolfswingGolferDataset"
...
val_evaluator = dict(type="CocoMetric", ann_file=f"{data_root}coco/hscc_golf_kps22_2d_test.json", nms_mode="none", score_mode="keypoint")
test_evaluator = dict(type="CocoMetric", ann_file=f"{data_root}coco/hscc_golf_kps22_2d_test.json", nms_mode="none", score_mode="keypoint")
